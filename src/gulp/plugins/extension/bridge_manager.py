...
if not GulpCollabBase.is_model_registered("GulpBridgeTaskEntry"):

    class GulpBridgeTaskEntry(GulpCollabBase, type=COLLABTYPE_BRIDGES_TASK):
        """
        Represents a bridge ingestion task managed by the bridge_manager plugin.
        """
        ...
        bridge_id: Mapped[str] = mapped_column(
            ...
            doc="id of the bridge this task belongs to",
        )
        plugin_params: Mapped[dict] = mapped_column(
            MutableDict.as_mutable(JSONB),
            doc="task specific parameters, passed via GulpPluginParameters structure",
        )
        status: Mapped[Optional[str]] = mapped_column(
            String, doc="ongoing|failed|done|stopped", default=None
        )
        error: Mapped[Optional[str]] = mapped_column(
            String, doc="error message if failed", default=None
        )
    ...
    # Get the already registered class from the registry
    GulpBridgeTaskEntry = GulpCollabBase.registry._class_registry["GulpBridgeTaskEntry"]


if not GulpCollabBase.is_model_registered("GulpBridgeEntry"):

    class GulpBridgeEntry(GulpCollabBase, type=COLLABTYPE_BRIDGES):
        """
        Represents a bridge registered with gulp via the bridge_manager plugin.
        """
        ...
        bridge_token: Mapped[str] = mapped_column(
            String,
            doc="the token generated by the bridge when registering, then used to call its other API",
        )
        status: Mapped[Optional[str]] = mapped_column(
            String, doc='"ready", "offline"', default="offline"
        )
        tasks: Mapped[Optional[list["GulpBridgeTaskEntry"]]] = relationship(
            "GulpBridgeTaskEntry",
    ...
    # Get the already registered class from the registry
    GulpBridgeEntry = GulpCollabBase.registry._class_registry["GulpBridgeEntry"]
...
class Plugin(GulpPluginBase):
    ...
    async def post_init(self, **kwargs):
        if self.is_running_in_main_process():
            MutyLogger.get_instance().debug(
                "adding api routes for bridge_manager plugin ..."
            )
                        ...
                        "content": {
                            "application/json": {
                                "example": {
                                    ...
                                    "timestamp_msec": 1701278479259,
                                    "req_id": "903546ff-c01e-4875-a585-d7fa34a0d237",
                                    "data": [GulpBridgeTaskEntry.example()],
                                }
                            }
                        }
                    }
            ...
            GulpServer.get_instance().add_api_route(
                "/get_bridge_status",
                self.get_bridge_status_handler,
                methods=["GET"],
                tags=["bridge"],
                response_model=JSendResponse,
                response_model_exclude_none=True,
                            ...
                            "application/json": {
                                "example": {
                                    ...
                                    "data": {
                                        ...
                                        "tasks": [
                                            {
                                                "id": "brdige_task_1234_abcd",
                                                "bridge_id": "bridge_id_1234_abcd",
                                                "parma": "GulpPluginParameters object",
            ...
            await GulpCollab.get_instance().create_table(GulpBridgeEntry.__table__)
            await GulpCollab.get_instance().create_table(GulpBridgeTaskEntry.__table__)
        else:
            MutyLogger.get_instance().debug(
                "bridge_manager initialized in worker process"
            )
    ...
    async def register_bridge_handler(
        ...
        url: Annotated[
            str,
            Query(
                description="bridge url in the format proto://[user:password]host:port"
            ),
        ],
        bridge_token: Annotated[
            str,
            Query(
                description="unique token generated by the bridge (will be stored by gulp to call other bridge APIs)"
            ),
        ],
        ...
        try:
            async with GulpCollab.get_instance().session() as sess:
                ...
                bridge = await GulpBridgeEntry.create_internal(
                    sess,
                    s.user.id,
                    name=name,
                    url=url,
            ...
            return JSONResponse(
                JSendResponse.success(req_id=req_id, data={"bridge_id": bridge.id})
            )
    ...
    async def unregister_bridge_handler(
        ...
        try:
            async with GulpCollab.get_instance().session() as sess:
                bridge: GulpBridgeEntry
                s, bridge, _ = await GulpBridgeEntry.get_by_id_wrapper(
                    sess, token, bridge_id, permission=GulpUserPermission.INGEST
                )
    ...
    async def create_start_ingestion_handler(
        ...
        token: Annotated[str, Depends(APIDependencies.param_token)],
        bridge_id: Annotated[
            str, Query(description="id of the bridge to use for ingestion")
        ],
        operation_id: Annotated[str, Query(description="operation id to ingest into")],
        plugin_params: Annotated[
            GulpPluginParameters,
            Body(
                description="anything needed by the bridge to perform the task on the target must be passed here (i.e. mapping, custom_parameters, ...)"
            ),
        ],
        req_id: Annotated[str, Depends(APIDependencies.ensure_req_id_optional)] = None,
    ) -> JSONResponse:
        ...
        try:
            ...
            async with GulpCollab.get_instance().session() as sess:
                ...
                _, bridge, _ = await GulpBridgeEntry.get_by_id_wrapper(
                    sess, token, bridge_id, permission=GulpUserPermission.INGEST
                )
                ...
                bridge_task_id = muty.crypto.hash_sha1(
                    f"{bridge_id}{operation_id}{params_str}"
                )
                existing = await GulpBridgeTaskEntry.get_by_id(
                    sess, bridge_task_id, throw_if_not_found=False
                )
                if existing and existing.status == GulpRequestStatus.ONGOING.value:
                    raise ObjectAlreadyExists(
                        f"bridge task already ongoing with id {bridge_task_id}"
                    )

                task: GulpBridgeTaskEntry
                if not existing:
                    # create task
                    task = await GulpBridgeTaskEntry.create_internal(
                        sess,
                        s.user.id,
                        ...
                        bridge_id=bridge_id,
                        plugin_params=(
                            plugin_params.model_dump() if plugin_params else {}
                        ),
                        status=None,
                    )
                else:
                ...
                async with aiohttp.ClientSession() as http_sess:
                    ...
                    async with http_sess.post(
                        bridge_url, headers=headers, json=payload, timeout=timeout
                    ) as r:
                        if r.status != 200:
                            # something wrong happened, report error and set task as failed
                            error_msg: str = await r.text()
                            MutyLogger.get_instance().error(
                                f"bridge start_ingestion error: {r.status} {error_msg}"
                            )
                            await task.update(
                                sess,
                                user_id=user_id,
                    ...
                    await task.update(
                        sess, user_id=user_id, status=GulpRequestStatus.ONGOING.value
                    )
                return JSONResponse(
                    JSendResponse.success(
                        req_id=req_id, data={"bridge_task_id": bridge_task_id}
                    )
                )
        except Exception as ex:
            raise JSendException(req_id=req_id) from ex

    async def _call_stop_ingestion(
        self,
        sess: AsyncSession,
        user_id: str,
        bridge: "GulpBridgeEntry",
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                if r.status != 200:
                    ...
                    MutyLogger.get_instance().error(
                        f"bridge stop_ingestion error: {r.status} {error_msg}"
                    )
                    await task.update(
                        sess,
                        user_id=user_id,
    ...
    async def stop_ingestion_handler(
        self,
        token: Annotated[str, Depends(APIDependencies.param_token)],
        bridge_task_id: Annotated[
            str,
            Body(
                description="a running bridge ingestion task returned from create_start_ingestion"
            ),
        ],
        req_id: Annotated[str, Depends(APIDependencies.ensure_req_id_optional)] = None,
    ) -> JSONResponse:
        ...
        try:
            ...
            async with GulpCollab.get_instance().session() as sess:
                task: GulpBridgeTaskEntry
                s, task, _ = await GulpBridgeTaskEntry.get_by_id_wrapper(
                    sess, token, bridge_task_id, permission=GulpUserPermission.INGEST
                )
                bridge = await GulpBridgeEntry.get_by_id(sess, task.bridge_id)
                user_id: str = s.user_id
                ...
                await task.update(
                    sess, user_id=user_id, status="stopped", req_id=req_id
                )
                return JSONResponse(
                    JSendResponse.success(
                        req_id=req_id, data={"bridge_task_id": bridge_task_id}
    ...
    async def delete_ingestion_handler(
        ...
        bridge_task_id: Annotated[
            str,
            Query(
                description="a running bridge ingestion task returned from create_start_ingestion (will be stopped and deleted)"
            ),
        ],
        req_id: Annotated[str, Depends(APIDependencies.ensure_req_id_optional)] = None,
    ) -> JSONResponse:
    ...
    async def list_ingestion_tasks_handler(
        ...
        flt: Annotated[
            ...
            Body(
                description="filter to apply (e.g. {'status': 'ongoing', 'bridge_id': 'abc123'})"
            ),
        ] = None,
        req_id: Annotated[str, Depends(APIDependencies.ensure_req_id_optional)] = None,
    ) -> JSONResponse:
        ...
        try:
            d = await GulpBridgeTaskEntry.get_by_filter_wrapper(
                token, flt or GulpCollabFilter(), permission=GulpUserPermission.INGEST
            )
    ...
    async def list_bridges_handler(
        ...
        flt: Annotated[
            Optional[GulpCollabFilter],
            Body(description="filter to apply (e.g. {'status': 'ready'})"),
        ] = None,
        req_id: Annotated[str, Depends(APIDependencies.ensure_req_id_optional)] = None,
    ) -> JSONResponse:
        params = locals()
        ...
        try:
            d = await GulpBridgeEntry.get_by_filter_wrapper(
                token, flt or GulpCollabFilter(), permission=GulpUserPermission.INGEST
            )
            return JSONResponse(JSendResponse.success(req_id=req_id, data=d))
        except Exception as ex:
    ...
    async def set_bridge_task_status_handler(
        ...
        status: Annotated[
            Literal["failed", "done", "ongoing", "canceled", "stopped"],
            Body(description="one of the GulpRequestStatus values to set for the task"),
        ],
        error: Annotated[
            Optional[str], Body(description="error message, in case of failure")
        ],
        req_id: Annotated[str, Depends(APIDependencies.ensure_req_id_optional)] = None,
    ) -> JSONResponse:
        params = locals()
    ...
    async def get_bridge_status_handler(
        self,
        token: Annotated[str, Depends(APIDependencies.param_token)],
        bridge_id: Annotated[
            str,
        ...
        async with GulpCollab.get_instance().session() as sess:
            ...
            try:
                bridge_status = await self._call_health_check(bridge)
            except Exception as ex:
                MutyLogger.get_instance().error(f"bridge health_check error: {ex}")
            ...
            return JSONResponse(
                JSendResponse.success(
                    req_id=req_id, data={"bridge_status": bridge_status}
                )
            )
    ...
    async def _call_health_check(
        ...
        """internal method to call bridge to stop ingestion for a task"""
        bridge_url = bridge.url.rstrip("/") + "/health_check"
        headers = {
            "Authorization": f"Bearer {bridge.bridge_token}",
            "Content-Type": "application/json",
        }
        async with aiohttp.ClientSession() as session:
            timeout = aiohttp.ClientTimeout(total=self._bridge_timeout_sec)
            async with session.post(bridge_url, headers=headers, timeout=timeout) as r:
                if r.status != 200:
                    # error occurred, raise
                    error_msg = await r.text()
                    MutyLogger.get_instance().error(
                        f"bridge health_check error: {r.status} {error_msg}"
                    )
                    return "offline"
