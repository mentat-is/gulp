...
class Plugin(GulpPluginBase):
    ...
    async def unregister_bridge_handler(
        ...
        try:
            async with GulpCollab.get_instance().session() as sess:
                ...
                s, bridge, _ = await GulpBridgeEntry.get_by_id_wrapper(sess, token, bridge_id, permission=GulpUserPermission.INGEST)
    ...
    async def create_start_ingestion_handler(
        ...
        try:
            ...
            async with GulpCollab.get_instance().session() as sess:
                ...
                _, bridge, _ = await GulpBridgeEntry.get_by_id_wrapper(sess, token, bridge_id, permission=GulpUserPermission.INGEST)
                ...
                existing = await GulpBridgeTaskEntry.get_by_id(sess, bridge_task_id, throw_if_not_found=False)
    ...
    async def _call_stop_ingestion(self, sess: AsyncSession, user_id: str, bridge: "GulpBridgeEntry", task: "GulpBridgeTaskEntry") -> str:
    ...
    async def stop_ingestion_handler(
        ...
        try:
            ...
            async with GulpCollab.get_instance().session() as sess:
                task: GulpBridgeTaskEntry
                s, task, _ = await GulpBridgeTaskEntry.get_by_id_wrapper(sess, token, bridge_task_id, permission=GulpUserPermission.INGEST)
    ...
    async def list_ingestion_tasks_handler(
        ...
        try:
            d = await GulpBridgeTaskEntry.get_by_filter_wrapper(token, flt or GulpCollabFilter(), permission=GulpUserPermission.INGEST)
    ...
    async def list_bridges_handler(
        ...
        try:
            d = await GulpBridgeEntry.get_by_filter_wrapper(token, flt or GulpCollabFilter(), permission=GulpUserPermission.INGEST)
            return JSONResponse(JSendResponse.success(req_id=req_id, data=d))
        except Exception as ex:
            raise JSendException(req_id=req_id) from ex
