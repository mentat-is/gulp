        ...
        **kwargs,
        ...
        for val in event.values:
            try:
                ...
                GulpLogger.get_instance().error(e)

        regkey["values"] = values

        # self.GulpLogger.get_instance().debug("VALUES: ",values)
        # apply mappings for each value
        fme: list[GulpMappingField] = []
        for k, v in muty.json.flatten_json(regkey).items():
            # self.GulpLogger.get_instance().debug(f"MAPPING: path:{d['path']} subkey_name:{d['subkey_name']} type:{d['type']} {k}={v}")
        ...
        # self.GulpLogger.get_instance().debug("processed extra=%s" % (json.dumps(extra, indent=2)))
    ...
    async def ingest_file(
        ...
        **kwargs,
        ...
        if record_to_gulp_document_fun is None:
            ...
            GulpLogger.get_instance().warning("using win_reg plugin record_to_gulp_document().")
