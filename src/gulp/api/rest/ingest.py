...
async def _request_handle_multipart(r: Request, req_id: str) -> dict:
    ...
    GulpLogger.get_instance().debug("request headers: %s" % (r.headers))
    ...
    GulpLogger.get_instance().debug("json_payload_part.headers=\n%s" % (json_payload_part.headers))
    GulpLogger.get_instance().debug("file_part.headers=\n%s" % (file_part.headers))
    fsize: int = 0
    ...
    try:
        ...
        GulpLogger.get_instance().debug("ingestion json payload: %s" % (payload_dict))
        if len(payload_dict) == 0:
            GulpLogger.get_instance().warning('empty "payload" part')
            payload_dict = None
    except:
        GulpLogger.get_instance().exception('invalid or None "payload" part: %s' % (payload))

    # download file chunk (also ensure cache dir exists)
    content_disposition = file_part.headers[b"Content-Disposition"].decode("utf-8")
    GulpLogger.get_instance().debug("Content-Disposition: %s" % (content_disposition))
    ...
    if fsize == total_file_size:
        ...
        GulpLogger.get_instance().info("file size matches, upload is already complete!")
    ...
    fsize = await muty.file.get_size(cache_file_path)
    GulpLogger.get_instance().debug("current size of %s: %d" % (cache_file_path, fsize))
    if fsize == total_file_size:
        GulpLogger.get_instance().info("file size matches, upload complete!")
        js = {"file_path": cache_file_path, "done": True}
    else:
    ...
    if payload_dict is not None:
        # valid payload
        js["payload"] = payload_dict
    GulpLogger.get_instance().debug("type=%s, payload=%s" % (type(js), js))
    return js
...
async def ingest_file_handler(
    ...
    # handle multipart request manually
    GulpLogger.get_instance().debug("headers=%s" % (r.headers))
    multipart_result = await _request_handle_multipart(r, req_id)
    done: bool = multipart_result["done"]
    file_path: str = multipart_result["file_path"]
    continue_offset: int = multipart_result.get("continue_offset", 0)
