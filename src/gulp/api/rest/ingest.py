...
if `preview_mode` is set, this function runs synchronously and returns the preview chunk of documents generated by the plugin, **without streaming data on the websocket, without saving data to the index nor counting data in the stats**.
""",
        ...
        if preview_mode:
            ...
            return JSONResponse(
                JSendResponse.success(
                    req_id=req_id, data={"status": str(status), "data": preview_chunk}
                )
            )
