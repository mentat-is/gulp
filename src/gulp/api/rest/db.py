...
- to use a custom `index template`, use a CURL as follows:

```bash
curl -X POST "http://localhost:8080/opensearch_init?index=test_idx&req_id=test_req" \\
    -H "Content-Type: multipart/form-data" \\
    -H "token: your_token" \\
    -F "index_template=@/home/valerino/repos/gulp/src/gulp/api/mapping/index_template/template.json"
```
""",
)
...
@router.post(
    ...
    summary="reset gulp.",
    description="""
- **WARNING: all data and collab will be erased!**
...
- rebase happens in the background and **uses one of the worker processess**: when it is done, a REBASE_DONE event is sent to the `ws_id` websocket.
"""
        ),
    ],
    ...
    async with GulpCollab.get_instance().session() as sess:
        ...
        s = await GulpUserSession.check_token(sess, token, [GulpUserPermission.INGEST])
        user_id = s.user_id
    ...
    async def worker_coro(kwds: dict):
        await GulpProcess.get_instance().process_pool.apply(_rebase_internal, kwds=kwds)
