...
class GulpQueryParameters(BaseModel):
    ...
    name: Annotated[
        str,
        Field(
            description="the name of the query, used as tag for notes if `create_notes` is set (autogenerated if not set).",
        ),
    ] = None
    group: Annotated[
        str,
    ...
    group_glyph_id: Annotated[
        Optional[str],
        Field(
            description="the query group glyph, if any: `GulpQueryGroupMatchPacket.glyph_id` in `WSDATA_QUERY_GROUP_MATCH` and notes generated by queries in this group (if `create_notes` is set) will use this glyph id",
        ),
    ] = None
    group_color: Annotated[
        Optional[str],
        Field(
            description="the query group color, if any: `GulpQueryGroupMatchPacket.color `WSDATA_QUERY_GROUP_MATCH` and notes generated by queries in this group (if `create_notes` is set) will use this color",
        ),
    ] = None
    sort: Annotated[
...
class GulpQuery(BaseModel):
    ...
    sigma_yml: Annotated[
        str, Field(description="if this is a sigma rule, the query in YAML format.")
    ] = None
    sigma_id: Annotated[
        str,
    ...
    group: Annotated[
        str, Field(description="the group this query belongs to, if any.")
    ] = None
    ...
    def __init__(
        ...
        if not name:
            # autogenerate name
            self.name = "query_%s" % (muty.string.generate_unique())
        super().__init__(
            q=q,
            name=name,
            sigma_yml=sigma_yml,
