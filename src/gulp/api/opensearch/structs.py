...
class GulpQueryParameters(BaseModel):
    ...
    name: Annotated[
        str,
        Field(
            description="the name of the query: for sigma queries, this is automatically set to the sigma rule name/title. If not set, a unique name is autogenerated.",
        ),
    ] = None
    group: Annotated[
        str,
        Field(
            description="the query group, if any: if set, `WSDATA_QUERY_GROUP_MATCH` is sent on the websocket if all queries in the same request matches.",
        ),
    ] = None
    sort: Annotated[
...
""",
    ...
    def __init__(self, **kwargs: Any) -> None:
        if "name" not in kwargs:
            # autogenerate name
            kwargs["name"] = "query_%s" % (muty.string.generate_unique())
