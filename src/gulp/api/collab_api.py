...
from gulp.api.collab.structs import PERMISSION_EDIT
from gulp.api.collab.structs import PERMISSION_DELETE
...
async def setup(force_recreate: bool = False) -> None:
    ...
    url = config.postgres_url()
    if force_recreate:
        logger().warning("force_recreate=True, dropping and recreating collab database ...")
        await _recreate_internal(url)
    else:
        if await db.exists(config.postgres_url()):
