...
class GulpUser(GulpCollabBase, type=GulpCollabType.USER):
        ...
        if "permission" in d:
            if not user_session.user.is_admin():
                ...
                raise MissingPermission(
                    "only admin can change permission, session_user_id=%s" % (user_session.user_id)
                )
    ...
    async def login(
        ...
        if GulpConfig.get_instance().is_integration_test():
            ...
            MutyLogger.get_instance().warning("using fixed token %s for integration test" % (token_id))
        else:
            # autogenerated
            token_id = None
