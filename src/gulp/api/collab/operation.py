...
class GulpOperation(GulpCollabBase, type=COLLABTYPE_OPERATION):
    ...
    async def add_context(
        ...
        # Acquire lock for the specific context id. `create_internal` commits the session
        # (which releases transaction-scoped advisory locks); acquiring the lock on the
        # `ctx_id` serializes context creation without relying on the operation-level lock
        # which could be released prematurely by nested commits.
        async with GulpContext.advisory_lock(sess, ctx_id):
