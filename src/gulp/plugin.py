...
from gulp.structs import (
    ...
    GulpIngestSourceDoneInternalEvent,
    GulpInternalEvent,
    GulpInternalEventResult,
    GulpInternalEventsManager,
...
class GulpPluginBase(ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.doc_value_cache: InternalDocValueCache = InternalDocValueCache()
    ...
    async def broadcast_ingest_internal_event(self) -> None:
        ...
        """
        assert self._stats
        ev: GulpIngestSourceDoneInternalEvent = GulpIngestSourceDoneInternalEvent(
            plugin=self.name,
        ...
        **kwargs,
        ...
        if not dry_run and enriched_chunk:
            updated, _, errors = await GulpOpenSearch.get_instance().update_documents(
                index, enriched_chunk, wait_for_refresh=last
            )
        elif not enriched_chunk:
            updated = 0
            errors = []
        else:
            # dry run, simulate ...
