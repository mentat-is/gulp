...
class GulpPluginBase(ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # in stacked plugins, this is called *after* _lower_record_to_gulp_document_fun
    ...
    async def _record_to_gulp_document(
        ...
        Args:
            record (any): the record to convert. in stacked plugins, this is always a GulpDocument generated by the lower plugin.
    ...
    async def _record_to_gulp_documents_wrapper(
        ...
        if self._lower_record_to_gulp_document_fun:
            ...
            # allow the upper plugin to further process the document
            doc = await self._record_to_gulp_document(doc, record_idx)
        else:
    ...
    async def _initialize(
        ...
        for p in self.specific_params() or []:
            ...
            self._specific_params[k] = plugin_params.get("model_extra", {}).get(
                k, p.default_value
            )
            logger().debug(
