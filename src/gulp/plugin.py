...
class PluginBase(ABC):
    ...
    async def sigma_plugin_initialize(
        ...
        """
        self.logger().debug("INITIALIZING SIGMA plugin=%s" % (self.name()))
    ...
    async def ingest_plugin_initialize(
        ...
        """
        # self.logger().debug("ingest_plugin_initialize: index=%s, pipeline=%s, mapping_file=%s, mapping_id=%s, plugin_params=%s" % (index, pipeline, mapping_file, mapping_id, plugin_params))
        self.logger().debug(
    ...
    def _type_checks(self, v: any, k: str, index_type_mapping: dict) -> any:
        ...
        if k not in index_type_mapping:
            # _logger.debug("key %s not found in index_type_mapping" % (k))
            return str(v)

        index_type = index_type_mapping[k]
        if index_type == "long":
            # self.logger().debug("converting %s:%s to long" % (k, v))
            if isinstance(v, str):
                if v.isnumeric():
                    return int(v)
                if v.startswith("0x"):
        ...
        if index_type == "keyword" or index_type == "text":
            # self.logger().debug("converting %s:%s to keyword" % (k, v))
            return str(v)

        if index_type == "ip":
            # self.logger().debug("converting %s:%s to ip" % (k, v))
            if "local" in v.lower():
                return "127.0.0.1"
            try:
        ...
        # self.logger().debug("returning %s:%s" % (k, v))
        return str(v)
    ...
    def _map_source_key(
        ...
        **kwargs,
        ...
        if index_type_mapping is None:
            index_type_mapping = {}
        # _logger.debug('len index type mapping=%d' % (len(index_type_mapping)))
        ...
        if source_key not in mapping_dict:
            # self.logger().error('key "%s" not found in custom mapping, mapping_dict=%s!' % (source_key, muty.string.make_shorter(str(mapping_dict))))
        ...
        for k in map_to_list:
            ...
            if is_numeric:
                ...
                # _logger.debug('***** str and is_timestamp, v nsec=%d' % (v))
            if fm.is_timestamp:
                ...
                # _logger.debug('***** timestamp nanos, v=%d' % (v))
                # _logger.debug('***** timestamp to millis, v=%d' % (vv))

            if fm.is_timestamp or fm.is_timestamp_chrome:
                # _logger.debug('***** timestamp or timestamp_chrome, v=%d' % (v))
                if v < 0:
                    # _logger.debug('***** adding invalid timestamp')
                    v = 0
            ...
            """
            self.logger().debug('FME LIST FOR THIS RECORD:')
            for p in fme_list:
                self.logger().debug(p)
            self.logger().debug('---------------------------------')
            """
    ...
    def _build_ingestion_chunk_for_ws(
        ...
        """
        # self.logger().debug("building ingestion chunk, flt=%s" % (flt))
        if not docs:
            return []
    ...
    async def _flush_buffer(
        ...
        # _logger.debug('flushing ingestion buffer, len=%d' % (len(self.buffer)))

        skipped, failed, failed_ar, ingested_docs = await elastic_api.ingest_bulk(
            self.elastic, index, self.buffer, flt=flt, wait_for_refresh=wait_for_refresh
        )
    ...
    def _record_failed(
        ...
        """
        # self.logger().exception("RECORD FAILED: source=%s, record=%s, ex=%s" % (muty.string.make_shorter(str(source),260), muty.string.make_shorter(str(entry),260), ex))
...
def load_plugin(
    ...
    **kwargs,
    ...
    """
    _logger.debug(
        "load_plugin %s, collab=%s, elastic=%s ..." % (plugin, collab, elastic)
    )

    m = plugin_cache_get(plugin)
    if m is not None:
        return m.Plugin(plugin, collab, elastic, **kwargs)
    ...
    mod: PluginBase = m.Plugin(path, collab, elastic, **kwargs)
    _logger.debug("loaded plugin: %s" % (mod.name()))
    ...
    for f in files:
        if "__init__" not in f and "__pycache__" not in f:
            try:
                ...
                _logger.exception(ex)
                _logger.error("could not load plugin %s" % (f))
                continue
...
def unload_plugin(mod: PluginBase) -> None:
    ...
    if mod is not None:
        ...
        _logger.debug("unloading plugin: %s" % (mod.name()))
        mod.cleanup()
        del mod
...
def plugin_cache_remove(plugin: str) -> None:
    ...
    if plugin in _cache:
        _logger.debug("removing plugin %s from cache" % (plugin))
...
def plugin_cache_add(m: ModuleType, name: str) -> None:
    ...
    if mm is None:
        _logger.debug("adding plugin %s (%s) to cache" % (name, m))
        _cache[name] = m
...
def plugin_cache_get(plugin: str) -> ModuleType:
    ...
    if p is not None:
        _logger.debug("found plugin %s in cache" % (plugin))
    return p
